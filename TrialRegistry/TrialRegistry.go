...
func (t *TrialRegistryChaincode) addEntry(stub *shim.ChaincodeStub, args []string) ([]byte, error) {
	...
	if len(args) < 2 {
		return nil, errors.New("addEntry operation must include two arguments, the trialDescriptionHash and trialPubKey")
	}
	entry := TrialRegistryHashMap{trialDescriptionHash: args[0], trialPubKey: args[1:]}
	...
	for i := range trials {
		if trials[i].trialDescriptionHash == entry.trialDescriptionHash {
			...
			for j := range entry.trialPubKey {
				isClinicRepeat := 0
				for k := range trials[i].trialPubKey {
					if entry.trialPubKey[j] == trials[i].trialPubKey[k] {
						isClinicRepeat = 1
					}
				}
				if isClinicRepeat == 0 {
					trials[i].trialPubKey = append(trials[i].trialPubKey, entry.trialPubKey[j])
				}
			}
			//create a local folder/file with url, upload the file to ipfs, get the hash, save in trialDescriptionHash
			trialPubKeyByte,_ := json.Marshal(trials[i].trialPubKey)
			err = stub.PutState(trials[i].trialDescriptionHash, trialPubKeyByte)
			if err != nil {
	...
	trialPubKeyByte,_ := json.Marshal(entry.trialPubKey)
	err = stub.PutState(entry.trialDescriptionHash, trialPubKeyByte)
	if err != nil {
		fmt.Printf("Error putting state %s", err)
		return nil, fmt.Errorf("put operation failed. Error updating state: %s", err)
	}
...
// getTrialClinics is used to return the trialPubKey of the trialDescriptionHash queried
